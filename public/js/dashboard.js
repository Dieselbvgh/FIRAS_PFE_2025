...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function fillGrypeTable(gr){
    ...
    gr.matches.forEach(m=>{
      ...
      const trEl = document.createElement('tr');
      trEl.innerHTML = `
        <td>${(m.artifact && (m.artifact.name||m.artifact.version)) || (m.package && m.package.name) || ''}</td>
        <td>${sev}</td>
      ...
      const poll = async () => {
        ...
        if (j.actions && j.actions.length > 0) {
          ...
          j.actions.forEach(action => {
            ...
            const sim = action.simulated ? ' (simulated)' : '';
            displayText += `   ${status} ${action.action}${sim}\n`;
            if (action.newImage) {
              displayText += `      → New image: ${action.newImage}\n`;
            }
          });
        }
  ...
  async function loadAlerts(){
    ...
    j.forEach(a=>{
      ...
      tr.innerHTML = `
        <td>${a.id}</td>
        <td>${a.source||a.checker||''}</td>
        <td>${a.severity||''}</td>
        <td>${(a.summary||'').slice(0,120)}</td>
        <td>${a.created_at||''}</td>
        <td>${a.status||''}</td>
